...
ind_date<-which(as.numeric(format(Opie_date, "%Y"))>="2006" & as.numeric(format(Opie_date, "%Y"))<="2006")
...
Spgrid_df_list <- list(ind_year,ind_month,Spgrid_df,t_mean,SpPDF,temperature_grid)
}
...
for(i in c(1:length(Wales_post_codes.sp)))
{
    ...
    for(i in c(1: length(Opie_data[,1])))
    {
    ...
    if(length(Opie_in)>1)
    {
      larger_area<-which(Wales_areas[Opie_in]==max(Wales_areas[Opie_in]))
      Opie_in<-Opie_in[larger_area]
    }
    
    if(length(Opie_in_corrected)>1)
    {
      larger_area<-which(Wales_areas[Opie_in_corrected]==max(Wales_areas[Opie_in_corrected]))
      Opie_in_corrected<-Opie_in_corrected[larger_area]
      }
    
    
    if(length(Opie_in)!=0)
    {
        Opie_in_Wales[i]<-Opie_in} else{
        
        if(length(Opie_in_corrected)!=0)
        {
            Opie_in_Wales[i]<-Opie_in_corrected
            #which(as.character(Wales_post_codes$LABEL)==substrCorr(Opie_data$PC[i]))
      #  print(c(i,which(as.character(Wales_post_codes$LABEL)==substrCorr(Opie_data$PC[i]))))
            } else {
            Opie_in_Wales[i]<-NA
        }
...
Opie_date_Wales<-as.Date(Opie_data[Opie_in_Wales,]$date, "%d/%m/%Y")

year_Wales<-unique(as.numeric(format(Opie_date_Wales, "%Y")))
month_Wales<-unique(as.numeric(format(Opie_date_Wales, "%m")))

#Spgrid_df_temporal<-list(nrow=length(month_Wales),ncol=length(year_Wales))
Spgrid_df_temporal<-list(month_Wales,year_Wales,temperature_temporal_data_base(month_Wales,year_Wales))
...
t_mean<-
  matrix(temperature_temporal_data_base(month_Wales,year_Wales)[[4]],nrow=length(month_Wales),ncol=length(year_Wales))

#SpPDF<- data.frame(
  #temperature_temporal_data_base(month_Wales,year_Wales_part)[[5]],month_Wales,year_Wales)


for(i in c(1: length(Opie_in_Wales)))
{
  
  
  #t_mean<-temperature_temporal_data_base(month_Wales,year_Wales)[[4]]
  
  year_Wales_part<-unique(as.numeric(format(Opie_date_Wales[i], "%Y")))
  index_year_Wales<-which(year_Wales[c(1:length(year_Wales))]==year_Wales_part)
  month_Wales<-unique(as.numeric(format(Opie_date_Wales[i], "%m")))
  
  #Spgrid_df<-temperature_temporal_data_base(month_Wales,year_Wales)[[3]]
  #temperature_in_Wales<- over(Wales_post_codes.sp[Opie_in_Wales[i]],Spgrid_df,returnList=TRUE)
  #temp_in_Wales[i]<-mean(temperature_in_Wales[[i]]$temperature_grid)
  
  SpPDF<-temperature_temporal_data_base(month_Wales,year_Wales_part)[[5]]
  temp_grid<-temperature_temporal_data_base(month_Wales,year_Wales_part)[[6]]
  tt<-over(SpPDF,gCentroid(Wales_post_codes.sp[Opie_in_Wales[i]], byid=FALSE, id = NULL),returnList=FALSE)
  temp_in_Wales[i]<-temp_grid[( which(tt==TRUE))]
  
  if(is.na(temp_in_Wales[i]))
  {
    temp_in_Wales[i]<-t_mean[month_Wales,index_year_Wales]# ths because some small postcode near  the sea where T=NA in the cell
    
  }
          print(c(paste(round(100*i/length(Opie_in_Wales),2),"%"),length(Opie_in_Wales),"Wales"))
}
...
for(i in c(1:length(England_post_codes.sp)))
{
    ...
    if(length(Opie_in)>1)
    {
    larger_area<-which(England_areas[Opie_in]==max(England_areas[Opie_in]))
    Opie_in<-Opie_in[larger_area]
    }
      
      if(length(Opie_in_corrected)>1)
    {
      larger_area<-which(England_areas[Opie_in_corrected]==max(England_areas[Opie_in_corrected]))
      Opie_in_corrected<-Opie_in_corrected[larger_area]
      }
    ...
    if(length(Opie_in)!=0)
    {
        Opie_in_England[i]<-Opie_in} else{
        
        if(length(Opie_in_corrected)!=0)
        {
            Opie_in_England[i]<-Opie_in_corrected
            #which(as.character(Wales_post_codes$LABEL)==substrCorr(Opie_data$PC[i]))
      #  print(c(i,which(as.character(Wales_post_codes$LABEL)==substrCorr(Opie_data$PC[i]))))
            } else {
...
Opie_date_England<-as.Date(Opie_data[Opie_in_England,]$date, "%d/%m/%Y")

year_England<-unique(as.numeric(format(Opie_date_England, "%Y")))
month_England<-unique(as.numeric(format(Opie_date_England, "%m")))

Spgrid_df_temporal<-list(month_England,year_England,temperature_temporal_data_base(month_England,year_England))


t_mean<-
  matrix(temperature_temporal_data_base(month_England,year_England)[[4]],nrow=length(month_England),ncol=length(year_England))



for(i in c(1: length(Opie_in_England)))
{
  
  
  #t_mean<-temperature_temporal_data_base(month_Wales,year_Wales)[[4]]
  
  year_England_part<-unique(as.numeric(format(Opie_date_England[i], "%Y")))
  index_year_England<-which(year_England[c(1:length(year_England))]==year_England_part)
  month_England<-unique(as.numeric(format(Opie_date_England[i], "%m")))
  
  
  SpPDF<-temperature_temporal_data_base(month_England,year_England_part)[[5]]
  temp_grid<-temperature_temporal_data_base(month_England,year_England_part)[[6]]
  tt<-over(SpPDF,gCentroid(England_post_codes.sp[Opie_in_England[i]], byid=FALSE, id = NULL),returnList=FALSE)
  temp_in_England[i]<-temp_grid[( which(tt==TRUE))]
  
  if(is.na(temp_in_England[i]))
  {
    temp_in_England[i]<-t_mean[month_England,index_year_England]
    
  }
 print(c(paste(round(100*i/length(Opie_in_England),2),"%"),length(Opie_in_England),"Wales"))
}
...
for(i in c(1:length(Scotland_post_codes.sp)))
{
    ...
    if(length(Opie_in)>1)
    {
     larger_area<-which(Scotland_areas[Opie_in]==max(Scotland_areas[Opie_in]))
      Opie_in<-Opie_in[larger_area]
       }
    ...
    if(length(Opie_in_corrected)>1)
    {
      larger_area<-which(Scotland_areas[Opie_in_corrected]==max(Scotland_areas[Opie_in_corrected]))
      Opie_in_corrected<-Opie_in_corrected[larger_area]
      } 
        
    if(length(Opie_in)!=0)
        {
            Opie_in_Scotland[i]<-Opie_in} else{
       if(length(Opie_in_corrected)!=0)
        {
            Opie_in_Scotland[i]<-Opie_in_corrected
                       } else {
...
Opie_date_Scotland<-as.Date(Opie_data[Opie_in_Scotland,]$date, "%d/%m/%Y")

year_Scotland<-unique(as.numeric(format(Opie_date_Scotland, "%Y")))
month_Scotland<-unique(as.numeric(format(Opie_date_Scotland, "%m")))


Spgrid_df_temporal<-list(month_Scotland,year_Scotland,temperature_temporal_data_base(month_Scotland,year_Scotland))


t_mean<-
  matrix(temperature_temporal_data_base(month_Scotland,year_Scotland)[[4]],nrow=length(month_Scotland),ncol=length(year_Scotland))



for(i in c(1: length(Opie_in_Scotland)))
{
  
  
 
  year_Scotland_part<-unique(as.numeric(format(Opie_date_Scotland[i], "%Y")))
  index_year_Scotland<-which(year_Scotland[c(1:length(year_Scotland))]==year_Scotland_part)
  month_Scotland<-unique(as.numeric(format(Opie_date_Scotland[i], "%m")))
  
 
  SpPDF<-temperature_temporal_data_base(month_Scotland,year_Scotland_part)[[5]]
  temp_grid<-temperature_temporal_data_base(month_Scotland,year_Scotland_part)[[6]]
  tt<-over(SpPDF,gCentroid(Scotland_post_codes.sp[Opie_in_Scotland[i]], byid=FALSE, id = NULL),returnList=FALSE)
  temp_in_Scotland[i]<-temp_grid[( which(tt==TRUE))]
  
  if(is.na(temp_in_Scotland[i]))
  {
    temp_in_Scotland[i]<-t_mean[month_Scotland,index_year_Scotland]#
    
  }
   print(c(100*i/length(Opie_in_Scotland),"Scotland"))
}
